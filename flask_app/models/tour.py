...
class Tour:
    ...
    @classmethod
    def save(cls, data):
        query = "INSERT INTO places (place_name,  category, price_range, schedule, location, description,reference_place , image,user_id) VALUES ( %(place_name)s, %(category)s, %(price_range)s,%(schedule)s,%(location)s,%(description)s,%(reference_place)s, %(image)s, %(user_id)s )"
    ...
    # Este metodo me logra traer las img
    @classmethod
    def get_image_by_admin(cls, formulario):
        query = "SELECT places.*, FROM places LEFT JOIN users ON users.id = places.user_id WHERE places.id = %(id)s"
        results = connectToMySQL('tour').query_db(query, formulario)
        return results
